...
const COPY = {
  nl: {
    ...
    email: "hoi@salonora.nl",
    emailLabel: "Mail ons",
    contactLabel: "Naar contact",
    addressLabel: "info@salonora.nl",
  },
  en: {
...
export function FaqContactBanner({ lang }: { lang: Locale }) {
  const copy = COPY[lang];
  const contactHref = buildLocalePath(lang, "contact");
  ...
  return (
    <aside className="faq-contact-banner w-full" aria-label={copy.title}>
      <div className="flex w-full flex-col items-center gap-6 rounded-2xl bg-linear-to-br from-brand to-brand-strong px-5 py-10 text-center text-white shadow-[0_16px_48px_-14px_rgba(21,41,81,0.35)] sm:px-10 sm:py-12">
        ...
        <div className="faq-cta-actions grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
          <Link href={`mailto:${copy.email}`} className={BTN_CLASS}>
            {copy.emailLabel}
          </Link>
          <Link href={contactHref} className={BTN_CLASS}>
            {copy.contactLabel}
          </Link>
        </div>

        <p className="text-sm font-medium text-white/85">
          <a href="mailto:info@salonora.nl" className="underline-offset-2 hover:underline">
            {copy.addressLabel}
          </a>
        </p>
      </div>
