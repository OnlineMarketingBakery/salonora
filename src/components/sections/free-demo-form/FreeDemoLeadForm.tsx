...
export function FreeDemoLeadForm({
  lang,
  ombFormId,
  successMode,
  redirectUrl,
  ...
  const showThankYou = Boolean(message) && !err && successMode === "inline";
  ...
  async function submit(payload: Record<string, string | number>) {
    ...
    try {
      ...
      setErr(
        !res.ok ? raw.slice(0, 280).trim() || c.errorGeneric : c.errorGeneric,
      );
      return;
    }
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        // Honeypot: do not use `name="company"` — browsers/password managers autofill it and abort submit with no UI feedback.
        const hp = String(fd.get("omb_hp_check") ?? "").trim();
        if (hp) return;
        ...
        const website_url =
          hasWebsite === "yes"
            ? String(fd.get("website_url") ?? "").trim()
            : "";
    ...
    >
      ...
      {showThankYou ? (
        ...
        >
          ...
          <button
            ...
            className="flex h-[50px] w-full cursor-pointer items-center justify-center gap-2 rounded-[12px] bg-gradient-to-b from-brand to-brand-strong text-sm font-medium leading-[1.54] text-white shadow-[0_6px_20px_color-mix(in_srgb,var(--palette-brand)_35%,transparent)] transition-[transform,filter,opacity] hover:brightness-[1.03] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60 disabled:active:scale-100"
          >
            {isSubmitting ? (
              <>
                <SubmitSpinner />
      ...
      {!showThankYou && message && !err ? (
        <p className="mt-5 text-center text-sm text-emerald-700" role="status">
          {message}
        </p>
      ) : null}
      {!showThankYou && err ? (
