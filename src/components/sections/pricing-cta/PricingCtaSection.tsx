...
export function PricingCtaSection({ section, lang }: { section: PricingCtaSectionT; lang: Locale }) {
  return (
    <section className="border-t border-sky-100 py-16 md:py-24">
      <Container>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          {section.pricingCards.map((c, i) => (
            <div key={i} className="rounded-2xl border-2 border-dashed border-sky-200 bg-white/80 p-6">
              <h3 className="text-lg font-bold text-[#0c1d3a]">{c.title}</h3>
              {c.description && <RichText html={c.description} className="mt-2 text-sm text-slate-600" />}
        ...
        {section.bottomContactText && <RichText html={section.bottomContactText} className="mt-10 text-center text-sm text-slate-500" />}
      </Container>
    </section>
  );
}
