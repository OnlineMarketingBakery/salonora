...
- **`combined_strengths`** — Figma **1090:47** (“Group 595”): solid brand panel + `left_rows`; three plain white cards (`content_cards`, Figma shadow); optional `accent` in ACF is normalized but not rendered; flat `navy-deep` footer bar (`footer_logo`, `footer_text`). Section surface `palette-surface`, cards `palette-white`.
