...
- **`combined_strengths`** — Figma **1090:47** (“Group 595”): solid brand panel with `title` + `left_rows` (image + description); stacked white `content_cards` (optional `accent` **brand** \| **rose** for light blue/pink top+bottom hairlines, else auto-alternate); full-width footer (`footer_logo`, `footer_text`) with edge-bright horizontal gradient.
