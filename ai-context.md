...
- **`combined_strengths`** — Figma **1090:47** (“Group 595”): solid brand panel + `left_rows`; white story cards with **brand/rose corner accents** (CMS `accent`, else auto-alternate) peeking at **top-right** and **bottom-left** behind the card; flat `navy-deep` footer (`footer_logo`, `footer_text`). Section `palette-surface`.
