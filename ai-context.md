...
`hero`, `cards`, `cost_comparison`, `benefits_grid`, `pricing_packages`, `guarantee_split`, `story_split`, `image_intro_split`, `salon_value_proposition`, `why_owners_choose`, `why_salonora_different`, `why_salonora_anders`, `testimonials`, `announcement_bar`, `process_steps`, `how_it_works_steps`, `scrolling_ticker`, `faq_contact_split`, `form_embed`, `latest_posts`, `cta`, `pricing_cta`, `rich_text`, `faq` — must stay in sync with `section-registry.ts`.
