...
- **Single case study** — Figma **879:27**: mirrors **`PostTemplate`** (breadcrumbs, two-column TOC + author, meta + share, **`post-article-body`**, optional CF7, flexible sections). CPT ACF: **`case_study_project_label`**, **`case_study_lead`**, **`case_study_outcome_metrics`** (full set on the brand metrics bar; overview still uses first three), **`show_toc`**, **`breadcrumb_parent`**, **`show_related_case_studies`**, **`featured_form`**. Breadcrumb archive segment from **`getCaseStudyArchiveSlug`** (`WORDPRESS_CASE_STUDY_PAGE_SLUG*`; default **`case-studies`**).
