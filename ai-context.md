...
| File                                          | Role                                                                                                                                                                                                                                                      |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wordpress/.../acf-json/group_*.json`         | **ACF Local JSON** (one file per field group; canonical source for field definitions). Deploy via **`npm run plugins:deploy -- --with-theme`**. Recovery: **`docs/acf-field-group-recovery.md`**. |
...
npm run validate:sections  # section pipeline sync check
npm run plugins:deploy     # SSH deploy omb-headless-core + omb-form-builder (-- --with-theme for acf-json)
```
...
`WORDPRESS_API_URL`, `WORDPRESS_BASE_URL`, `NEXT_PUBLIC_SITE_URL`, `HOMEPAGE_SLUG` / `HOMEPAGE_SLUG_NL` / `HOMEPAGE_SLUG_EN`, `WORDPRESS_BLOG_PAGE_SLUG` / `WORDPRESS_BLOG_PAGE_SLUG_NL` / `WORDPRESS_BLOG_PAGE_SLUG_EN` (blog archive slug segment for post breadcrumbs; defaults to `blog`), `WORDPRESS_CASE_STUDY_PAGE_SLUG` / `WORDPRESS_CASE_STUDY_PAGE_SLUG_NL` / `WORDPRESS_CASE_STUDY_PAGE_SLUG_EN` (case study archive segment for single-case-study breadcrumbs; defaults to `case-studies`), `WP_MENU_{PRIMARY|FOOTER|LEGAL}_{NL|EN}` (or `WORDPRESS_MENU_*`), `WORDPRESS_APPLICATION_USER`, `WORDPRESS_APPLICATION_PASSWORD`, `REVALIDATION_SECRET`, `NEXT_PUBLIC_DEFAULT_CF7_FORM_ID`, `WORDPRESS_SERVICE_REST_BASE`, `WORDPRESS_TESTIMONIAL_REST_BASE`, `WORDPRESS_CASE_STUDY_REST_BASE`, `DEFAULT_LOCALE`, `SUPPORTED_LOCALES`, `PLOI_SSH_HOST`, `PLOI_SSH_USER`, `PLOI_PLUGINS_PATH`, `PLOI_THEMES_PATH`, `PLOI_MU_PLUGINS_PATH`.

## Section registry keys (reference)

`hero`, `cards`, `combined_strengths`, `cost_comparison`, `benefits_grid`, `pricing_packages`, `guarantee_split`, `guarantees_promise_split`, `growth_plans_split`, `story_split`, `why_we_do_this`, `partner_intro_split`, `origin_story_split`, `our_promises`, `founder_story_split`, `founders_banner`, `who_we_are_for`, `is_this_for_you`, `features_checklist`, `media_text_checklist`, `audience_promo_card`, `blog_post_overview`, `case_study_overview`, `image_intro_split`, `salon_value_proposition`, `why_owners_choose`, `why_salonora_different`, `why_salonora_anders`, `testimonials`, `announcement_bar`, `process_steps`, `how_it_works_steps`, `scrolling_ticker`, `steps_with_media`, `design_showcase_grid`, `feature_highlight_grid`, `feature_highlight_split`, `talk_dual_cards`, `problem_solution`, `team_behind_salonora`, `faq_contact_split`, `form_embed`, `latest_posts`, `cta`, `pricing_cta`, `pricing_dual_cards`, `rich_text`, `faq`, `case_study_chapter`, `case_study_product_shot`, `case_study_client_review`, `case_study_conversion_cta` — must stay in sync with `section-registry.ts`.

- **`our_promises`** — Figma **693:395** (“Frame 2147229263”): white section, centered **`title`**, repeater **`items`** (`icon`, select **`icon_accent`** `brand` \| `rose`, **`title`**, WYSIWYG **`description`**) in a responsive four-column grid with circular icon discs.

- **`is_this_for_you`** — Figma **1325:38** (“Frame 2147230004”): white section, left column = **`title`**, **`subtitle`**, repeater **`checklist`** (`item`, optional **`icon`**; section **`list_default_icon`** fallback), WYSIWYG **`footer_note`**, link **`button`** + optional **`button_trailing_icon`**; right = **`image`** with rotated brand backing panel (same visual pattern as `audience_promo_card`).

- **`steps_with_media`** — Figma **1322:32** (“Frame 2147230002”): brand disc + browser chrome mockup with **`browser_image`**; **`title`**; repeater **`steps`** (`number`, select **`icon_color`** `blue` \| `pink`, **`title`**, WYSIWYG **`description`**); link **`cta_link`** for the pill CTA. Uses `reveal-item` scroll animation like other sections.

- **`audience_promo_card`** — Figma **692:196** (“Frame 2147229635”): white rounded card with shadow; left column = tilted `--palette-brand` panel + masked photo; right = optional **`badge_text`**, multi-line **`title`**, WYSIWYG **`description`**, repeater **`features`** / **`item`**, link **`button`**, optional **`button_trailing_icon`** on the CTA.
...
- **`case_study_overview`** — Figma **866:4217** (“Case studies overview”): same mesh hero band as blog + **`title`** / **`intro`** + optional **`hero_stats`** (KPI cards use **`grid_columns`** 1–4 on large screens). Case study **card list** below is fixed at **3 columns** on large screens for now. **`grid_rows`** sets how many rows of cards to load per page: **`postsPerPage`** = `min(50, 3 × grid_rows)`. Optional **`show_featured`** + **`featured_case_study`**, **`read_more_label`**. No search field. Grid cards use CPT fields **`case_study_project_label`** and **`case_study_outcome_metrics`** (up to three rows for the featured block); list REST requests use **`acf_format=standard`** so `acf` is present when the field group is REST-enabled. Enable **“Case study archive page”** (`is_case_study_archive`) on the WordPress page for **`?page=`** / **`?s=`**; CPT **`case_study`** REST base defaults to `case_study` or override with **`WORDPRESS_CASE_STUDY_REST_BASE`**. Single entries resolve before services/posts and render **`CaseStudyTemplate`**.

- **Single case study** — Figma **879:27**: like **`PostTemplate`** (breadcrumbs, two-column TOC + author, **`post-article-body`**, optional CF7, flexible sections) but **no** bordered author/date/read-time/share row under the lead (Figma goes straight to the metrics bar). Main column **`lg:max-w-[53.6875rem]`** (859px). **Lead → KPI strip** spacing **24px** (`mt-6` on metrics); strip matches Figma **152px** min height, **54×46** padding, **42px** horizontal separation + **1px** white dividers, **10px** label/value stack, **34px / 58px line** values. **Inhoudsopgave** lists each **Chapter** heading plus **Client review** (custom **Section heading** or default “Klantenrecensie” / “Client review”). **`case_study_product_shot`**: required **Image**, optional **Title** above, optional **Description** (WYSIWYG) below, optional **Show divider under block** (same hairline as **Chapter**); plain rounded figure, **no** brand-blue frame (legacy **`caption`** → **Title**). CPT ACF: **`case_study_project_label`**, **`case_study_lead`**, **`case_study_outcome_metrics`** (full set on the brand metrics bar; overview still uses first three), **`show_toc`**, **`breadcrumb_parent`**, **`show_related_case_studies`**, **`featured_form`**. Breadcrumb archive segment from **`getCaseStudyArchiveSlug`** (`WORDPRESS_CASE_STUDY_PAGE_SLUG*`; default **`case-studies`**). **Over de auteur** card matches Figma **902:819** (`PostAuthorCard`: Outfit **24px semibold / 1.1** title + greeting, **16px regular / 1.4** bio **`#435780`**, **89px** avatar, **14px** surface **`#ebf3fe`**, **14px** radius, **30/33/30** padding, **6px** social gaps, **30px** white icon wells + **18px** brand-blue glyphs). Data from **Author** + **`/wp/v2/users/{id}?lang=`**; optional user meta **`omb_author_facebook`** / **`omb_author_instagram`** (REST **`omb_author_social`**); **`url`** → LinkedIn vs website globe. **`rest_prepare_user`** maps Polylang bios into **`description`**.

- **Single post template** — Figma **1643:54** ([Salonora New Design](https://www.figma.com/design/FugnwTE5ugAf4YMK0g6ghT/Salonora-New-Design?node-id=1643-54)): `PostTemplate` + `src/components/templates/post/*` — optional ACF **`post_lead`**, **`breadcrumb_parent`**, **`show_toc`**, **`show_related_posts`**. Two-column layout (**307px** TOC, **~969px** main), TOC only (no author card). **Shared tail:** **Global Settings → Templates** options (`omb-templates-settings`, field **`blog_single_sections`**, clone of page flexible layouts) via **`fetchBlogSingleTailSections`**; OMB **`/globals`** may expose **`templates`**; fallback: layout page **`is_blog_single_layout`**. **`faq_contact_split`** before **`PostRelatedGrid`** (`#post-faq`). Related: **`fetchRelatedPostCards`** by post category. **`docs/blog-single-template.md`**, **`docs/n8n-blog-publishing.md`**.
